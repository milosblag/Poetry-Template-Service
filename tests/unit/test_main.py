...
async def test_lifespan_normal():
    ...
    with patch("app.main.logger", mock_logger):
        ...
        assert any("starting up" in args[0] for args, _ in mock_logger.info.call_args_list)
        # Check second call contains "shutting down"
        assert any("shutting down" in args[0] for args, _ in mock_logger.info.call_args_list)
...
async def test_lifespan_exception():
    ...
    with patch("app.main.logger", mock_logger):
        ...
        assert mock_logger.info.call_count == 2
        mock_logger.info.assert_any_call(f"Application starting up in {settings.ENVIRONMENT} environment...")
